...
#[derive(BorshSerialize, BorshDeserialize)]
pub struct RaydiumAmmSwapArgs { pub instruction: u8, pub amount_in: u64, pub minimum_amount_out: u64 }
...
pub fn wrap_sol_instructions(user: &Pubkey, lamports: u64) -> Result<Vec<Instruction>> {
    ...
    let create = spl_associated_token_account::instruction::create_associated_token_account_idempotent(user, user, &wsol, &tok_prog);
...
pub fn create_ata_idempotent(user: &Pubkey, mint: &Pubkey, token_2022: bool) -> Result<Instruction> {
    ...
    Ok(spl_associated_token_account::instruction::create_associated_token_account_idempotent(user, user, mint, &prog))
}
