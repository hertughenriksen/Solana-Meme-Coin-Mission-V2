...
impl TelegramScanner {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        let session = Session::load_file_or_create("./secrets/telegram.session")?;
        ...
        if !client.is_authorized().await? {
            ...
            self.authorize(&client, &cfg.phone).await?;
        }
    ...
    async fn authorize(&self, client: &grammers_client::Client, phone: &str) -> Result<()> {
        ...
        match client.sign_in(&token, code.trim()).await {
            Ok(_) => {
                ...
                client.session().save_to_file("./secrets/telegram.session")?;
            }
