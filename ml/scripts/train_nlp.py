...
EXAMPLES = [
    ...
    ("honeypot [MINT] can't sell contract blocked", 0),
...
CRYPTO_VOCAB = {"lp":"liquidity pool","mc":"market cap","mcap":"market cap","devs":"developers",
                "kol":"key opinion leader","ape":"buy aggressively","aped":"bought aggressively",
                "rugged":"rug pulled","lfg":"lets fucking go"}

def preprocess(text):
    text = re.sub(r'\b[1-9A-HJ-NP-Za-km-z]{43,44}\b','[MINT]',text)
    text = re.sub(r'https?://\S+','[URL]',text)
    text = re.sub(r'\$[A-Z]{2,10}\b','[TICKER]',text)
    text = text.lower()
    for abbr,exp in CRYPTO_VOCAB.items(): text=re.sub(r'\b'+abbr+r'\b',exp,text)
...
def train_model(output_dir, db_url=None):
    ...
    for epoch in range(1,EPOCHS+1):
        ...
        log.info(f"Epoch {epoch} | loss={tr_loss/len(tr_ld):.4f} | F1={f1:.4f} | best={best_f1:.4f}")
        if patience>=3: break
    ...
    dummy_ids=torch.zeros(1,MAX_LEN,dtype=torch.long); dummy_mask=torch.ones(1,MAX_LEN,dtype=torch.long); dummy_tt=torch.zeros(1,MAX_LEN,dtype=torch.long)
